new Vue({
  el: '#app',
  data: {
    inputMsg: '', chatCollapsed: false,
    clockTimer: null, nowTs: Date.now(), sending: false, isFullscreen: false, isMuted: false,
    room: {
  ...
  computed: {
    currentDuration() {
      return this.formatDuration(this.room.liveBaseSeconds + Math.floor((this.nowTs - this.bootAt) / 1000));
    },
    canChat() { return this.room.chatOpen && !this.isMuted; },
    muteTip() {
      if (this.isMuted) return '您已被禁言，无法发送消息。';
      if (!this.room.chatOpen) return '当前课堂暂不开放聊天，仅支持观看直播。';
      return '';
    }
  },
  created() {
  ...
  methods: {
    refreshIcons() { if (window.lucide) this.$nextTick(() => window.lucide.createIcons()); },
    sendMsg() {
      ...
      setTimeout(() => {
        ...
        this.inputMsg = '';
        this.sending = false;
        this.$nextTick(() => { const el = this.$refs.msgList; if (el) el.scrollTop = el.scrollHeight; this.refreshIcons(); });
      }, 220);
    },
    startLive() { this.room.started = true; this.$nextTick(() => this.refreshIcons()); },
    toggleMute() { this.isMuted = !this.isMuted; },
    toggleChat() { this.chatCollapsed = !this.chatCollapsed; this.refreshIcons(); },
    toggleFullscreen() {
      const el = document.querySelector('.room-page');
      if (!document.fullscreenElement && el && el.requestFullscreen) el.requestFullscreen();
      else if (document.exitFullscreen) document.exitFullscreen();
    },
    handleFullscreenChange() { this.isFullscreen = !!document.fullscreenElement; this.refreshIcons(); },
    startClock() { this.nowTs = Date.now(); this.clockTimer = setInterval(() => { this.nowTs = Date.now(); }, 1000); },
    now() { const d = new Date(); return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; },
    formatDuration(s) { return `已直播 ${String(Math.floor(s/60)).padStart(2,'0')}:${String(s%60).padStart(2,'0')}`; }
  }
});
